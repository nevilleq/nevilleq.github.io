...
Recall that a pipe operator `%>% or \>` takes the output from the l.h.s. of the pipe and passes it as an input to the right hand side of the pipe. So when using a general dplyr pipeline flow, such as 
...
Derived tables are going to feel unnatural compared to the way we can manipulate data with `mutate` in `dplyr`, but they are necessary for more complex queries. What is a derived table you ask? Similar to piping in `R` where `f(a) %>% g(.)` means takes the output of `f(a)` and passes it as an input to `g()`, when performing a mutate statement  


## 1.3 Sub-queries  

Very similar in syntax and flavour to derived tables, sub-queries can only contain *one column* and are generally used within `filter`/WHERE/HAVING. For example, in `R` we can pass data manipulations, functions, and other variables within the `filter` without thinking anything much of it -- 
...
*Answer* -- because under the hood, SQL is actually a collection of hyper-efficient algorithms which seek to optimally complete the query you've requested. Since it doesn't pull data into memory until the query is complete (the whole point of db languages), if it weren't procedural, I imagine the complexity and compute time would become astronomical. Plus, as the name suggests, a set of relatively few procedures, instructions, or functions, is great for computers and maximizes efficient db querying -- whereas a language like R which is much more flexible and user-friendly *but* does not use pre-defined memory types or allocation, is *interpretted* rather than compiled, and inherently pulls all data into memory prior to a `dplyr` pipe (by default, however there are many ways to connect with db's & use SQL without every leaving the R ecosystem!)  

### Q: How do these algorithms check memory and wrangle data without pulling it all into active memory (RAM)?

*Answer* -- No idea!  

But if I was to speculate wildly, familiarity with C++ and dynamic versus static memory allocation (pointers) would suggest that it may have something to do dynamic memory allocation. In that vein, a very high level view suggests that each individual algorithm (behind the SQL procedures) is accessing small pieces of data with dynamic memory allocation *very very very quickly* and then just as quickly removing that allocation to make way for the next one. They "compete" with each other in the sense that some algorithms will be faster than others given the specific type of data or request -- in a very capitalist sort of way that produces the most efficient solution to wrangle data larger than memory or any tabular data very quickly.

### Q: SQL is super efficient, but what can't it do?  

*Answer* -- As far as I can tell, it only handles tabular data (maybe it can handle list columns but again, additional complexity in data structure comes at the cost of compute time). There are many types of data that don't fit into tabular data, MRI/fMRI data is now generally stored in standardized NIFTI format and we also usually zip those NIFTI files up to save space (a single subject, even zipped, can take up 1 gig of memory in fMRI as it's sort of a collection of MRI's in time). That data is also not "human readable" and it seems to me like anything not human readable can't really be dealt with via SQL because again, it's not really tabular or able to be looked at in a spreadsheet. 

Even though SQL has limitations, in a production environment or Shiny app setting where speed of data wrangling and access is critical, there's nothing that can really compete with it in terms of efficiency -- which is why it's still in use almost everywhere 50+ years after it's inception. Again, I think there are way to many versions with basic or fundamental syntax conflicts because it wasn't developed cohesively (but companies have tried to make it slightly easier to use, SQLite for ex.). 
...
*Answer* -- If you want to be a well-rounded data scientist in a production environment, or even just want to speed up your own data wrangling workflow with SQL data bases, *yes you need to know SQL* (to the point that you can use the internet to create a more complex query if you need to for work). As mentioned above, yes there are tons of ways to handle data bigger than memory without leaving the `R` ecosystem but SQL is used almost everywhere whereas `R` is much less prevalent (i.e. though the number of `R` users measured by proxy through Github `R` package development has skyrocketed in the last decade).  

### Q: When is `dbplyr` sufficient and when do I need to use SQL?  

While `dplyr` was surely built with SQL procedures in mind (the flow/verbs are very similar) and `dbplyr` will work for most basic to intermediate SQL queries, there's no such thing as a free lunch. One of the biggest downsides to relying on a translator is that there are lots of data wrangling functions, workflows, and the ability to supply user defined functions almost anywhere that I use everyday in `R` but can't use with `dbplyr` because there is no SQL equivalent (or nobody has gotten around to writing a transator/wrapper for it yet, looking at you `stringr`). *However*, though it may not be able to generate the exact SQL query you need in a giving setting, you can certainly build a pretty comprehensive skeleton for a query with `show_query` (the syntax is also generally sub-optimal for both readability and reproducibility due to translation so copy and pasting queries is inadvisable).  
